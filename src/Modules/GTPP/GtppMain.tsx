...
export default function GtppMain(props: GtppMainProps) {
  ...
  function HeaderFilters() {
    return (
      <div className="d-flex w-100 align-items-center justify-content-start gap-5 my-2 py-2">
        ...
        <div className="d-flex gap-4 mb-3 flex-wrap">
          ...
          {true && (
            <div>
              <label className="form-label mb-1">Vincular tema à tarefa:</label>
              <select className="form-select" value={props.idTheme} onChange={(e) => props.getIdTheme(e.target.value)}>
                <option value="" hidden>
                  Selecione
                </option>
                {props.themeList?.map((theme) => (
        ...
        <div className="flex-grow-1 d-flex flex-column justify-content-between align-items-start h-100 overflow-hidden">
          <div className="d-flex flex-column justify-content-between w-100">
            <div className="flex-grow-1 me-2 w-100">
              {props.isHeader ? <CardUser {...props.userLog} name={props.userLog.name} /> : null}
            </div>
            <div className="d-flex justify-content-between">
              <div className="d-flex flex-row mt-2 gap-2 flex-wrap">
                {props.listButtonInputs.map((btn: any, idx: number) => (
                  <InputCheckButton key={`btn_header_gtpp_${idx}`} {...btn} />
                ))}
              </div>
                    ...
                    <ColumnTaskState
                      ...
                      addTask={() => {
                        props.setModalPageElement(
                          <Cardregister
                            reloadtask={props.loadTasks}
                            assistenceFunction={() => props.setModalPage(false)}
                            onClose={() => props.setModalPage(false)}
                          />
                        );
