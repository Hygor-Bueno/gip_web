...
export default function CommentItem({ item, isMe, isAdmin, userPhoto, userName, editingId, deleteComment, isHighlighted }: CommentItemProps) {
  return (
    <div data-comment-id={item.id} className={`d-flex mb-3 align-items-end animate__animated animate__fadeIn ${isMe ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`d-flex flex-column overflow-hidden ${isMe ? 'align-items-end' : 'align-items-start'}`} style={{ maxWidth: '80%', width: editingId === item.id ? '100%' : 'auto' }}>
        <div className={`p-2 px-3 shadow-sm ${isHighlighted ? 'gtpp-comment-bubble-bounce' : ''}`} style={{ backgroundColor: '#2b2d42', borderRadius: isMe ? '20px 20px 2px 20px' : '20px 20px 20px 2px', border: '1px solid rgba(0,0,0,0.1)', width: '100%' }}>
