...
export type HiddenColumnKey = typeof listColumnsOcult[number];
...
export type CustomTagKey = keyof typeof customTagsActive;
...
export type ValueFormatter = (value: unknown, row?: ActiveRow) => string;
...
export const customValueActive: Record<string, ValueFormatter> = {
  ...
  place_purchase: (value: unknown): string => {
    try {
  ...
  list_item: (value: unknown): string => {
    try {
      return value != null ? String(value) : "";
    } catch {
      return "";
  ...
  status_active: (value: unknown): string => {
    try {
      return String(value) === "1" ? "Ativo" : "Inativo";
    } catch {
      return "";
    }
  },
